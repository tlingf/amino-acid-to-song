...
1. **aa_mapping.js** — Data layer. Exports globals: `MAPPINGS` (4 mapping configs), `AM` (active aa→note map), `AN` (aa→full name), `GR` (aa→property group), `GC` (group→color config), `FR` (note→Hz), `HP` (hydrophobic set), `HY` (Kyte-Doolittle hydrophobicity scale), `COMPLEXES` (4 binding pair definitions with contacts), `A3` (3-letter codes), `AB` (blurbs). The `setMapping(id)` function switches the active mapping.
...
4. **pdb_data.js** — Pre-downloaded PDB structures as raw strings. Exports: `PDB_DATA` (keyed by PDB ID: 1YCR, 4INS, 2ZTA, 1BRS).
